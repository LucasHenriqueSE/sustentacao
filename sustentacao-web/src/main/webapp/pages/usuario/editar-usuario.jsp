	...
	<div class="section">
		<div class="container">
			...
			<form action="/sustentacao/painel/usuario/editar" method="POST"
				role="form">
				<br> <input id="id" name="id" value="${idUsuario}"
					type="hidden">
				<div class="row">
					...
					<div class="form-group col-md-4">
						<label>Usuario:</label> <input type="email" class="form-control"
							id="usuario" value="${usuario.email}" name="email" required>
					</div>
					<div class="form-group col-md-offset-2 col-md-4">
							...
							<c:forEach var="perfil" items="${perfil}">
								<c:choose>
									<c:when test="${usuario.perfil.id == perfil.id}">
										<option value="${perfil.id}" label="${perfil.descricao}"
											selected />
									</c:when>
									<c:otherwise>
										<option value="${perfil.id}" label="${perfil.descricao}" />
							...
							<c:choose>
								<c:when test="${usuario.ativo}">
									<label for="sim"><input id="sim" name="ativo"
										value="true" type="radio" required checked /> Sim</label>
									<label for="nao"><input id="nao" name="ativo"
										value="false" type="radio" required /> Não</label>
								</c:when>
								<c:otherwise>
									<label for="sim"><input id="sim" name="ativo"
										value="true" type="radio" required /> Sim</label>
									<label for="nao"><input id="nao" name="ativo"
